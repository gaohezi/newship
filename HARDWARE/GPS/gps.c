...
//////-------------------------------------------------------------------------------main
 ...
 u8 USART1_TX_BUF[UART5_MAX_RECV_LEN]; 					//串口1,发送缓存区
nmea_msg gpsx; 											//GPS信息
...
int gps_qty;//gps数量
int bd_qty;//北斗数量
		char Longitude[40];//经度
		char Lattitude[40];//纬度
void Gps_Msg_Show(void)
{
	...
	sprintf((char *)dtbuf,"Longitude:%.5f %1c  \r\n ",	tp/=100000,gpsx.ewhemi);	//得到经度字符串
		...
		//u1_printf("Longitude:%.5f %1c   ",tp,gpsx.ewhemi);//发送接收到的数据到串口1
					longitude = tp;
//	u1_printf("longitude----:%.5f   \r\n",longitude);//发送接收到的数据到串口1
//					u1_printf("Longitude:%.5f %1c   ",longitude,gpsx.ewhemi);
	tp=gpsx.latitude;	   
	sprintf((char *)dtbuf,"Latitude:%.5f %1c   ",tp/=100000,gpsx.nshemi);	//得到纬度字符串
					latitude=tp;
	sprintf((char *)Lattitude,"Longitude:%.5f %1c   ",tp/=100000,gpsx.ewhemi);
...
//	u8 upload=0;	
		...
		{
	//		Uart1_SendStr(USART1_TX_BUF);
			rxlen=UART5_RX_STA&0X7FFF;	//得到数据长度
		...
		//		sprintf((char *)Longitude,"Longitude:%.5f %1c   ",longitude,gpsx.ewhemi);
//			u1_printf("longitude:%.5f	%1c	\r\n",longitude,gpsx.ewhemi);
//			u1_printf("latitude:%.5f	%1c	\r\n",latitude,gpsx.nshemi);
//			if(upload)
...
//		if(UART5_RX_STA&0X8000)		//接收到一次数据了
//		{
//			u1_printf("UART5_RX_BUF:%f		\r\n",UART5_RX_BUF);
//			rxlen=UART5_RX_STA&0X7FFF;	//得到数据长度
//			for(i=0;i<rxlen;i++)USART1_TX_BUF[i]=UART5_RX_BUF[i];	   
// 			UART5_RX_STA=0;		   	//启动下一次接收
//			UART5_RX_BUF[i]=0;			//自动添加结束符
